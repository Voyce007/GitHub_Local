...
This is the same matrix ci.yml generates from ci-cd/services.yaml -- if a
service is added there, add its build/curl lines here too. Signing
(Cosign) and release bundling only run on a tag push through the actual
GitHub Actions workflow and aren't practical to reproduce locally.
